...
type Clerk struct {
	...
	client map[int]map[int]*labrpc.Client
	id int
	me     int64
	serial int
	leader int
...
// send RPCs.
//
func MakeClerk(client map[int]map[int]*labrpc.Client ,me int,nmaster int,net *labrpc.NetWork) *Clerk {
	ck := new(Clerk)
	...
	ck.serial = 0
	ck.leader = 0
	masters := labrpc.MakeGroupSerClientAsList(ck.id%10,ck.id-ck.id%10,0,nmaster,net)
	ck.sm = shardmaster.MakeClerk(masters)
	...
	for {
		...
		if servers, ok := ck.config.Groups[gid]; ok {
			...
			for si := 0; si < len(servers); si++ {
				srv := ck.client[ck.id][StrToInt(servers[si])]
				var reply GetReply
